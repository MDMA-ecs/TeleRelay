...
class DB:
    ...
    def get_settings_updated_ts(self, key: str = "runtime") -> Optional[int]:
        with self._conn() as c:
            row = c.execute("SELECT updated_ts FROM settings WHERE key=?", (key,)).fetchone()
        if not row:
            return None
        try:
            return int(row[0])
    ...
    def get_events(
        ...
        q = (
            "SELECT id,ts,kind,source,msg_id,fp,proto,status,latency_ms,exit_ip,country_code,country,asn,org,detail "
    ...
    def get_kpis(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._conn() as c:
            ...
            test_ok = c.execute(
                "SELECT COUNT(*) FROM events WHERE ts>=? AND kind='test' AND status='ok'", (since,)
            ).fetchone()[0]
            test_fail = c.execute(
                "SELECT COUNT(*) FROM events WHERE ts>=? AND kind='test' AND status='fail'", (since,)
            ).fetchone()[0]
    ...
    def get_recent_stage_fail_count(
        self,
        fp: str,
